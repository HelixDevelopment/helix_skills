...
// integration test(s), following the SAME pattern (and the SAME
// SKILL_SYSTEM_TEST_DB_* environment contract) already established by
// internal/db/testdb_helper_test.go and internal/skill/migration_granularity_test.go.
// Each _test.go-only helper file in this codebase duplicates a small,
// package-local copy of this helper rather than exporting it from the db
// package, since these are test-only symbols and the db package's own
// copy is unexported test-file-scoped code that cannot be imported.
//
...
const workerRealMigrationsDir = "../../migrations"

func workerTestDBAdminConfig() (config.DatabaseConfig, bool) {
...
func workerSkipIfNoTestDB(t *testing.T) (config.DatabaseConfig, bool) {
	t.Helper()
	admin, ok := workerTestDBAdminConfig()
	if !ok {
		t.Skip("SKILL_SYSTEM_TEST_DB_HOST not set: this case requires a live " +
			...
			"registry-review ticker cycle's real effect on skill_registry rows " +
			"(research/p05_high_defect_fix_designs.md §4.4/§4.6; same boundary " +
			"as internal/db and internal/skill's own _RequiresLiveDatabase tests).")
		return config.DatabaseConfig{}, false
	}
	return admin, true
}

func workerCreateThrowawayDB(t *testing.T, admin config.DatabaseConfig) (config.DatabaseConfig, func()) {
	t.Helper()
